...
std::list<SOCKET>	g_listClient; // 연결된 클라이언트 소켓
...
DWORD WINAPI ThreadFunction(LPVOID pParam)
{
	...
	while ((nReceive = ::recv(hClient, szBuffer,
		sizeof(szBuffer), 0)) > 0)
	{
		puts(szBuffer);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//Ctrl + C를 눌렀을 때 이를 감지해 처리할 함수를 등록
